-- query left - median, inactive_listing, total_stays, avg-estimated_revenue_per_active_listings
select 
	...
	((count(case when has_availability = true then 1 end) - lag(count(case when has_availability = true then 1 end)) over (PARTITION BY listing_neighbourhood)) * 100)/lag(count(case when has_availability = true then 1 end)) over (PARTITION BY listing_neighbourhood) as active_listing_change,
...
group by listing_neighbourhood, TO_CHAR(TO_TIMESTAMP(date , 'YYYY-MM-DD'), 'MM/YYYY')